angular.module('pokeradar').factory('Map', function($cordovaGeolocation, Pokevision, LoadingScreen, $ionicPopup){
  ...
  function loadMarkers(callback) {
    ...
    Pokevision.getNewPokemon(currentLocMarker.position.lat(), currentLocMarker.position.lng()).then(function(pokemon){
      ...
      for (var i = 0; i < records.length; i++) {
        ...
        if(exclude[record.pokemon_id]) continue;
      ...
      if(!pokemonFound || !Array.isArray(pokemonFound)) {
        LoadingScreen.hide();
        var alertPopup = $ionicPopup.alert({
           title: 'Uh-Oh!',
           template: 'Pokévision is having some issues... please try again in a bit <br> <br> <a browse-to="https://pokevision.com/">Pokévision</a> <br> <br> <a browse-to="https://twitter.com/PokeVisionGo">Twitter</a>'
         });
